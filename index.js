...
const vcard = 'BEGIN:VCARD\n' // JAN DI UBAH
            ...
            + 'END:VCARD'
prefix = '!'
blocked = ['6285891927691@s.whatsapp.net']
		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[❗] PERINTAH INI KHUSUS USER *MOD FANA BOT*',
					benned: 'Anda Ke Band Silahkan Hubungi Owner Agar Membuka Band Anda',
					ownerG: '[❗] Lu spe owner? ini fitur khusus owner si ❌',
					ownerB: '[❗] Lu spe owner? ini fitur khusus owner su ❌',
					userB: `──「 DAFTAR 」──\nHalo Kak !\nMaaf Kak Kamu belum Terdaftar Sebagai Teman Fana Daftar Untuk Berteman Dengan Fana bot dengam cara, \n\nCommand : ${prefix}daftar nama|umur\nContoh : ${prefix}daftar Fana|17\n\n──「 FANA BOT 」──`,
			...
			const ownerNumber = ["6285891927691@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"6285891927691@s.whatsapp.net"]
			const adminbotnumber = ["6285891927691@s.whatsapp.net"]
			const frendsowner = ["6285891927691@s.whatsapp.net"]
			const premium = ["6285891927691@s.whatsapp.net"]
			...
			switch(command) {
				...
				if (!isPublic) return reply(mess.only.public)
					...
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
					client.sendMessage(from, '*FANA GROUP*\n\nLink : https://chat.whatsapp.com/K2izM4SkMh8L7Jm1jzmlQ3*', text, { quoted: mek })
					break
					case 'funmenu':
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
			       ...
			       case 'valxlove':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT FANA ✨* 」\n*│+wa.me/6285891927691*╰──────*「 *FANA* 」*────*\n\n*_JIKA INGIN MENJADI ADMIN  FANA BOT_*\n*_Ketik /iklan_*' })
					break
					case 'premiumcek':
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
					sa = await getBuffer(`https://i.ibb.co/Gv5W97v/IMG-20210106-WA1995.jpg`)
					client.sendMessage(from, sa, image, { quoted: mek, caption: '*╭────*「 *PREMIUM USER👑* 」\n*│+ wa.me/6285891927691*╰──────*「 *FANA* 」*────*\n\n*_JIKA INGIN MENJADI PREMIUM USER FANA BOT_*\n*_Ketik #daftarvip*' })
					break
					case 'cekmod': 
		...
		case 'iri':
			...
			if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
                 ...
                 client.sendMessage(from, 'itu anjing ku eh owner ku ><',MessageType.text, { quoted: mek} )
...
if (isBanned) return reply(mess.only.benned)    
               ...
               client.reply(from, `Waalaikumsalam ${pushname}:)`)
				...
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					client.sendMessage(from, '*Gopay :* _085891927691_\n*Saweria :* _https://saweria.co/FANAbot2412\n*Ovo :* _082137299043_', text, { quoted: mek })
					break
                case 'ttp':
                if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
				...
				if (!isUser) return reply(mess.only.userB)
                    ...
                    client.sendMessage('6285891927691@s.whatsapp.net', options, text, {quoted: mek})
 ...
 if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
				 data = fs.readFileSync('./src/FANAjokes.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
                hasil = await getBuffer(randKey.result)
                client.sendMessage(hasil, image, mek, '\`\`\`FANAJOKES\`\`\`')
				break
					case 'kata':
					if (!isPublic) return reply(mess.only.public)
...
if (isBanned) return reply(mess.only.benned)    
 ...
 if (isBanned) return reply(mess.only.benned)    
			...
			if (!isPublic) return reply(mess.only.public)
					...
					mentions('╔══✪〘 Mention All 〙✪══'+teks+'╚═〘  FANA BOT 〙', members_id, true)
					break
				case 'clearall':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
	...
	.then(() => {client.sendMessage(from, options, text)})
				...
				//frendowner
				case `addadmin`:
                   if (!isOwner) return reply('Perintah ini hanya bisa di gunakan oleh Owner FANA!', id)
                   for (let i = 0; i < mentionedJidList.length; i++) {
                   adminNumber.push(mentionedJidList[i])
                   fs.writeFileSync('./lib/database/admin.json', JSON.stringify(adminNumber))
                   reply('Success Menambahkan Admin FANA!')
				   }
                   break
				case 'wait':
				if (isBanned) return reply(mess.only.benned)    
